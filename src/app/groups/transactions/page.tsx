...
import { ArrowLeft, Receipt, FileText, Edit2, Search, Zap, Coins, Loader2 } from "lucide-react";
    ...
    return (
      <div className="flex h-screen items-center justify-center p-8 bg-background">
        <Card className="glass-card p-12 text-center rounded-[3rem] border-dashed border-white/10 max-w-md">
          <h2 className="text-2xl font-black uppercase tracking-tight text-glow">Missing Sector ID</h2>
          <p className="text-[10px] text-muted-foreground uppercase font-bold tracking-widest mt-4 mb-8">Synchronize a specific group node to scan its historical ledger.</p>
          <Button onClick={() => router.push("/groups")} className="w-full h-12 rounded-2xl font-black uppercase tracking-widest text-[10px] bg-primary glow-primary">View All Nodes</Button>
        </Card>
      </div>
    );
  }
